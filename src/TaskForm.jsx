...
import { addWorkdays, computeStatus, STATUS_COLOR, STATUS_BG, today } from './helpers'
import './TaskForm.css'
    ...
    <div className="task-form">
      ...
      <div className="task-form__grid">
        <div className="task-form__field">
          ...
          <select className="form-select" value={form.itName} onChange={e => set('itName', e.target.value)}>
        ...
        <div className="task-form__field">
          ...
          <input className="form-input" type="number" min="0.5" step="0.5" value={form.manday} onChange={e => set('manday', e.target.value)} />
        </div>
        ...
        <div className="task-form__field--pair">
          ...
          <div>
            <label className="field-label">
              End Date
              <span className={`field-label__lock-toggle ${endLocked ? 'field-label__lock-toggle--locked' : 'field-label__lock-toggle--auto'}`} onClick={() => setEndLocked(l => !l)}>
        ...
        <div className="task-form__field--pair">
          <div>
            <label className="field-label">Target UAT <span className="field-label__hint">(optional)</span></label>
            <input className="form-input" type="date" value={form.targetUAT} onChange={e => set('targetUAT', e.target.value)} />
          </div>
          <div>
            <label className="field-label">Target LIVE <span className="field-label__hint">(optional)</span></label>
            <input className="form-input" type="date" value={form.targetLive} onChange={e => set('targetLive', e.target.value)} />
          </div>
        </div>
        ...
        <div className="task-form__field--full">
          ...
          <div className="progress-row">
            <input className="progress-row__range" type="range" min="0" max="100" step="5" value={form.progress} onChange={e => set('progress', Number(e.target.value))} />
            <input className="progress-row__number" type="number" min="0" max="100" step="1" value={form.progress} onChange={e => {
        ...
        <div className="task-form__field">
          <label className="field-label">Status <span className="field-label__hint">(auto or override)</span></label>
          <div className="status-row-wrap">
            <select className="form-select status-select" value={form.status} onChange={e => {
              const val = e.target.value
              set('status', val)
      ...
      <div className="task-form__actions">
        {onCancel && <button className="btn-ghost" onClick={onCancel} disabled={saving}>Cancel</button>}
        <button className="btn-submit" onClick={handleSubmit} disabled={saving} style={{ opacity: saving ? 0.7 : 1 }}>
