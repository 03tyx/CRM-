...
function TasksSection({ itName, tasks, createTask, updateTask, deleteTask, saving, canEdit }) {
  const [showForm, setShowForm] = useState(false)
  const [editing,  setEditing]  = useState(null)
...
function ITDeploymentSection({ itName, deployments = [], entries, getRows, saveRows, syncToMainDeployment, isSaving, canEdit }) {
                            ...
                            <tr key={d.id} className={isLastDetail ? 'dep-tr--last-detail' : 'dep-tr--mid-detail'}>
                              ...
                              <td className="dep-td" style={{ minWidth: 200 }}>
                                {canEdit ? (
                                  ...
                                  <span style={{ fontSize: 12, color: '#94a3b8' }}>{d.remark || '—'}</span>
                                )}
                              </td>
                              <td className="dep-td" style={{ minWidth: 130 }}>
                                {canEdit ? (
                              ...
                              <td className="dep-td" style={{ minWidth: 80, textAlign: 'center' }}>
                                {canEdit ? (
                                  ...
                                  <span style={{ fontSize: 12, color: d.testingRequired ? '#22c55e' : '#ef4444' }}>
                                    {d.testingRequired ? 'Yes' : 'No'}
                                  </span>
                                )}
                              </td>
                              <td className="dep-td" style={{ minWidth: 70 }}>
                                {canEdit ? (
                                  ...
                                  <span style={{ fontSize: 12, color: '#94a3b8' }}>{d.md || '—'}</span>
                                )}
                              </td>
                              <td className="dep-td" style={{ minWidth: 130 }}>
                                {canEdit ? (
                                  ...
                                  <span style={{ fontSize: 12, color: '#94a3b8' }}>{d.pic || '—'}</span>
                                )}
                              </td>
                              <td className="dep-td" style={{ minWidth: 130 }}>
                                {canEdit ? (
                                  ...
                                  <span style={{ fontSize: 12, color: '#ef4444' }}>{d.liveDate || '—'}</span>
                                )}
                              </td>
                              {canEdit && (
                                <td className="dep-td" style={{ width: 60, textAlign: 'center' }}>
...
function ITMemberCard({ itName, tasks = [], deployments = [], itEntries = [], getRows, saveRows, syncToMainDeployment, createTask, updateTask, deleteTask, saving }) {
  const { canEditMember } = useAuth()
  const canEdit = canEditMember(itName)
