    ...
    <div>
      ...
      <div className="dashboard-kpi-grid">
        {[
          ...
          <div key={k.label} className="kpi-card" style={{ borderColor: `${k.color}25` }}>
            <div className="kpi-card__icon">{k.icon}</div>
            <div className="kpi-card__value" style={{ color: k.color }}>{k.val}</div>
      ...
      <div className="dashboard-mid-grid">
        ...
        <div className="dash-panel">
          <h3 className="dash-panel__title">👥 Team Availability</h3>
          <div className="avail-col-header">
            <div>NAME</div><div>THIS WEEK</div><div>NEXT WEEK</div>
          </div>
          {availability.map(a => (
            <div key={a.m} className="avail-row">
              <div className="avail-row__name">
                {a.m}
                {a.delayed > 0 && <span title={`${a.delayed} delayed`} style={{ fontSize: 10 }}>🔴</span>}
              </div>
              {[a.thisWeekFree, a.nextWeekFree].map((free, i) => (
                <div key={i} className={`avail-cell ${free ? 'avail-cell--free' : 'avail-cell--busy'}`}>
                  <div className={`avail-dot ${free ? 'avail-dot--free' : 'avail-dot--busy'}`} />
                  {free ? 'Free' : 'Busy'}
                </div>
              ))}
            </div>
        ...
        {/* ── Status Overview ── */}
        <div className="dash-panel">
          <h3 className="dash-panel__title dash-panel__title--mb">📊 Status Overview</h3>
          {Object.entries(statusDist).map(([s, c]) => (
            <div key={s} className="status-row">
              <div className="status-row__header">
                <Badge label={s} color={STATUS_COLOR[s]} bg={STATUS_BG[s]} />
                <span className="status-row__count">{c} task{c !== 1 ? 's' : ''}</span>
              </div>
              <div className="progress-track progress-track--sm">
                <div
                  className="progress-fill"
                  style={{ width: `${(c / total) * 100}%`, background: STATUS_COLOR[s] }}
                />
              </div>
            </div>
          ))}
      ...
      <div className="workload-panel">
        <h3 className="dash-panel__title dash-panel__title--mb">🏋️ Current Workload</h3>
        <div className="workload-list">
          {availability.map(a => (
            <div key={a.m} className="workload-row">
              <div className="workload-row__name">{a.m}</div>
              <div className="workload-row__bar-wrap">
                <div className="progress-track progress-track--lg">
                  <div
                    className="progress-fill"
                    style={{
                      width: `${Math.min(100, (a.load / 5) * 100)}%`,
                      background: a.load === 0 ? '#22c55e' : a.load <= 2 ? '#3b82f6' : a.load <= 3 ? '#f59e0b' : '#ef4444',
                      transition: 'width 0.5s ease',
                    }}
                  />
              ...
              <div className="workload-row__count">{a.load} active</div>
              {a.delayed > 0 && (
                <div className="workload-row__delayed">⚠️ {a.delayed} delayed</div>
              )}
            </div>
          ))}
        </div>
      ...
      {/* ── Active This Week ── */}
      <div className="active-week-panel">
        <h3 className="dash-panel__title dash-panel__title--mb">🗓️ Active This Week</h3>
        <div className="active-week-list">
          {tasks
            .filter(t => {
              const s = computeStatus(t)
              return t.startDate <= thisWkEnd && t.endDate >= thisWkStart && s !== 'Completed'
            })
            .map(t => {
              const st = computeStatus(t)
              return (
      ...
      {delayed.length > 0 && (
        <div className="delayed-alert">
          ...
          {delayed.map(t => (
            <div key={t.id} className="delayed-alert__row">
              <span className="delayed-alert__member">{t.itName.split(' ')[0]}</span>
              <span className="delayed-alert__sep">—</span>
              <span className="delayed-alert__name">{t.project}</span>
              <span className="delayed-alert__due">Due {t.endDate}</span>
