...
const ROW_H   = 54   // base row height
const LABEL_W = 175  // sticky left panel width
const HEADER_H = 80  // header height (room for month label + day label + vertical line labels)
...
// ── Build all calendar days (incl weekends) in the range ─────────────────────
...
export default function GanttChart({ tasks = [], leaves = [] }) {
  ...
  // ── Time range: 2 months before today → 3 months after today ───────────────
  const { rangeStart, rangeEnd, cols } = useMemo(() => {
    const start = new Date(today)
    start.setMonth(start.getMonth() - 2)
    start.setDate(1)  // anchor to 1st of that month
    const end = new Date(today)
    end.setMonth(end.getMonth() + 3)
    end.setDate(0)    // last day of 3rd month ahead
  ...
  function dayIdx(dateStr) {
    return colIdxMap[dateStr] ?? -1
  }

  // Nearest idx: if date not in cols (weekend), find closest workday forward
  ...
  const byMember = useMemo(() => {
    ...
    tasks.forEach(t => {
      ...
      // Hide completed tasks finished more than 1 month ago
      if (st === 'Completed' && t.endDate && t.endDate < oneMonthAgo) return

      const effectiveEnd = t.endDate || rangeEnd
      // Include only if start OR end falls within range
  ...
  // ── AL grouped by member — only current/upcoming ──────────────────────────
  const alByMember = useMemo(() => {
    const map = {}
    IT_MEMBERS.forEach(m => { map[m] = [] })
    leaves.forEach(function (l) {
        if (!map[l.it_name]) return
        // Show if AL end date >= today (current or future)
        if (l.end_date >= today && l.start_date <= rangeEnd) {
          map[l.it_name].push(l)
        }
      })
    return map
  }, [leaves, rangeEnd])
  ...
  function rowHeight(m) {
    ...
    // AL label below member name adds ~16px, task bars stacked at 22px each
  ...
  return (
    ...
    {/* ── Outer scroll container ── */}
      <div style={{ overflowX: 'auto', overflowY: 'auto', maxHeight: '80vh' }}>
        <div style={{ minWidth: LABEL_W + cols.length * COL_W, position: 'relative' }}>
                  ...
                  <div key={m} style={{
                    ...
                    {myAL.map(al => (
                      <div key={al.id} style={{
                        ...
                        🌴 {fmtShort(al.start_date)}
                        {al.start_date !== al.end_date ? ` – ${fmtShort(al.end_date)}` : ''}
                      </div>
                    ))}
                  </div>
                )
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', height: '100%', position: 'absolute', top: 0, left: 0 }}>
                      {cols.map(d => {
                        ...
                        return (
                          <div key={d} style={{
                            minWidth: COL_W, width: COL_W, height: '100%',
                            background: d === today
                    ...
                    {/* AL highlight band (per-member, amber) */}
                    {myAL.map(al => {
                      const si = nearestIdx(al.start_date)
                      // find last col <= end_date
                      let ei = -1
                    ...
                    {todayIdx >= 0 && (
                      <div style={{
                        ...
                        background: '#3b82f6', zIndex: 5, pointerEvents: 'none',
                      }} />
                    )}

                    {/* +1 week vertical line */}
                    {weekIdx >= 0 && (
                      <div style={{
                        position: 'absolute',
                        left: weekIdx * COL_W + COL_W / 2 - 1,
                        top: 0, bottom: 0, width: 2,
                        background: '#f97316', zIndex: 5, pointerEvents: 'none',
                      }} />
                    )}
                    ...
                    {memberTasks.map((t, tIdx) => {
                      const effectiveEnd = t.endDate || rangeEnd
                      let si = nearestIdx(t.startDate)
                      let ei = nearestIdx(effectiveEnd)
                      ...
                      // Tooltip flips based on position in list
                      const flipUp = mIdx > IT_MEMBERS.length / 2
                      const ttStyle = {
                        ...tooltipStyle,
                        top:    flipUp ? 'auto' : '110%',
                        bottom: flipUp ? '110%' : 'auto',
                      }
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Tooltip */}
                          <div className="task-tooltip" style={ttStyle}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                              ...
                              <div style={{ color: '#94a3b8' }}>
                                ⏱ Manday: <span style={{ color: '#e2e8f0' }}>{t.manday|| '—'}</span></div>
                              {/* )} */}
                              <div style={{ color: '#94a3b8' }}>
                                📅 Start: <span style={{ color: '#e2e8f0' }}>{t.startDate || '—'}</span>
                              </div>
                              <div style={{ color: '#94a3b8' }}>
                                🏁 End: <span style={{ color: '#e2e8f0' }}>{t.endDate || '—'}</span>
                              </div>
                              <div style={{ color: '#94a3b8' }}>
                                📊 Progress: <span style={{ color: '#e2e8f0' }}>{t.progress || 0}%</span>
                              </div>
                              <div style={{ color: '#94a3b8' }}>
                                🏷 Status: <span style={{ color }}>{st}</span>
                              </div>
                            </div>
                          </div>
                        </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 14, flexWrap: 'wrap', padding: '0 4px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#94a3b8' }}>
          <div style={{ width: 2, height: 10, background: '#3b82f6' }} />
          Today
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#94a3b8' }}>
          <div style={{ width: 2, height: 10, background: '#f97316' }} />
          Today + 1 Week
        </div>
      </div>
    </div>
