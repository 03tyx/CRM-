...
export default function DeploymentBoard({
  ...
  const [justSaved,         setJustSaved]         = useState({})
  const [depRows,           setDepRows]           = useState({})
  const [showTestScenarios, setShowTestScenarios] = useState({})  // per deployment toggle
    ...
    <div>
      ...
      <div className="dep-card-list">
        {deployments.map(dep => {
          const expanded   = expandedId === dep.id
          const envColor   = ENV_COLOR[dep.environment] || '#64748b'
          const rows       = getPARows(dep.id)
          const totalCount = getAllRows(dep.id).length
          const showTS     = showTestScenarios[dep.id] || false
                ...
                <div className="dep-card__panel">
                  ...
                  <div style={{ marginBottom: 12 }}>
                    <button
                      ...
                      onClick={() => setShowTestScenarios(s => ({ ...s, [dep.id]: !showTS }))}
                    >
                      {showTS ? 'Hide Test Scenarios' : 'Show Test Scenarios'}
                    </button>
                  </div>
                    ...
                    <div className="dep-table-scroll">
                      <table className="dep-table">
                        ...
                        <tbody>
                          {rows.map((row, rowIdx) => {
                            const span = row.details.length
                            // Each remark detail may have an extra Test Scenarios row below it
                            // rowSpan for # and Task must account for that
                            return row.details.map((d, di) => {
                              const isSelfDisc   = d.discovery === 'self-discovered'
                              const isLastDetail = di === span - 1
                              ...
                              return (
                                <>
                                  ...
                                  >
                                    {di === 0 && (
                                      <td rowSpan={hasTS ? span * 2 : span} className="dep-td--span dep-td--num">
                                        {rowIdx + 1}
                                      </td>
                                    )}
                                    {di === 0 && (
                                      <td rowSpan={hasTS ? span * 2 : span} className="dep-td--span" style={{ minWidth: 200, verticalAlign: 'top' }}>
                                        <TaskCell value={row.task} onChange={task => patchRowTask(dep.id, row.id, task)} />
                                        <button className="btn-add-row" onClick={() => addDetail(dep, row.id)}>
                                          + add row
                                        </button>
                                      ...
                                      <td colSpan={7} style={{ padding: '6px 10px 10px', background: 'rgba(139,92,246,0.04)', borderTop: '1px dashed #1e293b' }}>
                                        ...
                                        {d.testScenario ? (
                                          <div
                                            ...
                                            dangerouslySetInnerHTML={{ __html: d.testScenario }}
                                          />
                                        ) : (
                                          <span style={{ fontSize: 11, color: '#475569', fontStyle: 'italic' }}>No test scenarios entered yet.</span>
                                        )}
                                      </td>
                                    </tr>
                                  )}
                                        ...
                                        return (
                                          <>
                                            <tr key={d.id} style={{ borderBottom: isLast && !showTS ? '1px solid #1e293b' : 'none' }}>
                                              ...
                                              {di === 0 && (
                                                <td rowSpan={(row.details?.length ?? 1) * tsSpan} style={{ width: 235, padding: 8, verticalAlign: 'top' }}>
                                                  {taskUrl
                                                    ? <a href={taskUrl} target="_blank" rel="noopener noreferrer" className="task-cell__link" style={{ fontSize: 12 }}>{taskLabel}</a>
                                                    : <span style={{ color: '#f1f5f9' }}>{taskLabel}</span>}
                                                </td>
                                            ...
                                            {showTS && (
                                              <tr key={`${d.id}-ts`} style={{ borderBottom: isLast ? '1px solid #1e293b' : 'none' }}>
                                                <td colSpan={6} style={{ padding: '4px 8px 8px', background: 'rgba(139,92,246,0.04)', borderTop: '1px dashed #1e293b' }}>
                                                  ...
                                                  {d.testScenario ? (
                                                    <div style={{ fontSize: 12, color: '#cbd5e1', lineHeight: 1.6 }} dangerouslySetInnerHTML={{ __html: d.testScenario }} />
                                                  ) : (
